...
//   // Após 3 segundos, inicia o fade out e remove a div
//   setTimeout(() => {
//     welcomeMessage.style.opacity = "0"; // Inicia o fade out
//     setTimeout(() => {
//       welcomeMessage.style.display = "none"; // Remove a div após o fade out
//     }, 1000); // Tempo para o fade out terminar
//   }, 3000); // Tempo que a mensagem ficará visível antes de desaparecer
// });
...
function displayReminders() {
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        if (eventDate.getMonth() === currentMonth && eventDate.getFullYear() === currentYear) {
            ...
            listItem.innerHTML = `<strong>${event.title}</strong> - ${event.description} em ${eventDate.toLocaleDateString()}`;
...
function createEventTooltip(date, month, year) {
    ...
    for (let i = 0; i < eventsOnDate.length; i++) {
        ...
        let eventDate = new Date(event.date);
        let eventText = `<strong>${event.title}</strong> - ${event.description} em ${eventDate.toLocaleDateString()}`;
